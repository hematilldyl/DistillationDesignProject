...
flash drum seperation using the SRK EOS
...
xlswrite('Dylan Hematillake Lab 6.xlsx',Results,2)
end
...
a=0.42747.*R.^2.*CPT(:,2).^2./CPT(:,1).*(1+(0.48508+1.55171.*CPT(:,3)-0.15613.*CPT(:,3).^2).*(1-sqrt((T+273.15)./CPT(:,2)))).^2;                                                                                      % Equation 12
b=0.08664.*R.*CPT(:,2)./CPT(:,1);  % Equation 33
...
Am=mf*(Aij*mf');
Bm=mf*B;  
poly=[1 -1 Am-Bm-Bm^2 -Am*Bm]; %Equation 34                                                 
Z=roots(poly);
if W==0
 Zm=min(Z);
else Zm=max(Z);
...
phi=exp(B./Bm.*(Zm-1)-log(Zm-Bm)-Am./Bm.*(2.*(Aij*mf')/Am-B./Bm).*log(1+Bm./Zm)); %Equation 35
