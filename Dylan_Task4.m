...
for i=1:num
    ...
    T0=40
    %initial guess
    mo=[0.5 0.1 0.02 0.02 0.01 T0]; 
  
    m=fsolve(@ProcessEquations,mo);
 
    x=z; %x=z because bubble point calculation
    y=m(1:n);
    T=m(n+1);
    Results(i,:) = [Pop(i) T]
...
xlswrite('Dylan Hematillake Lab 6.xlsx',Results,4)
end
...
x=z; %y=z because dew point calculation
y=m(1:n); 
T=m(n+1); 
